...
| `c6-oligos` | Read every oligo file under one or more directories and emit them as JSON. | — |
...
| `c6-rebase` | refresh src/data/rebase-enzymes.json from REBASE. | `--check` |
...
356 records in `sharables/generated/`, one per exported function that carries a JSDoc comment,
...
**82 exported functions have no record** (`c6-sharables --undocumented` lists them), because the
...
**Two types: `function` (309) and `datum` (47).** A `datum` is an exported constant that carries a
