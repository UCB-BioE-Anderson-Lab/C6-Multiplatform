...
269 records in `sharables/generated/`, one per exported function that carries a JSDoc comment,
...
**58 exported functions have no record** (`c6-sharables --undocumented` lists them), because the
...
**Two types: `function` (234) and `datum` (35).** A `datum` is an exported constant that carries a
