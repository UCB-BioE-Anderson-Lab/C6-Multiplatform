...
| `c6-scenarios` | the synthetic experiments the suite compiles: what each shape is and what it reaches. `--write <dir>` puts one on disk as an ordinary project folder. → §11 |
...
284 records in `sharables/generated/`, one per exported function that carries a JSDoc comment,
...
**Two types: `function` (244) and `datum` (40).** A `datum` is an exported constant that carries a
...
|---|---|---|
| `plan` | 79 | the planning stages and their helpers — naming, vessels, sessions, sources, injectors, decisions |
...
| `server`, `render`, `labplanner` | 49 | entry points |
...
| | before | after |
|---|---|---|
| experiments compiled end to end | 2 | 28 |
...
`src/labplanner/scenarios/` holds twenty-six named experiment shapes, each carrying **what** it is
...
a refusal a person can act on, and a third outcome is a defect.** Two scenarios are refused today
and both record the message they are refused with, so a refusal going away fails the test — the
golden snapshot's discipline applied to an outcome rather than to a page.
...
- **`from block` is the wrong header when the pick went into tubes**, and it now repeats the label
  beside it: a four-colony pick is four tubes, so the miniprep row reads `pGOLD-A | pGOLD-A`. The
  data is right — the tube you fetch really is the one of that name — and the redundancy is
  inherent to the naming convention. Where the pick fills a BLOCK the column still earns its place,
  saying `A1`, `B1`. Changing a header on a bench sheet is an editorial call, so it is flagged
  rather than taken.
