...
| `c6-report` | run every example and write what happened, as one page somebody reads. | `--answers` `--inventory` `--open` `--write` |
...
287 records in `sharables/generated/`, one per exported function that carries a JSDoc comment,
...
**Two types: `function` (247) and `datum` (40).** A `datum` is an exported constant that carries a
