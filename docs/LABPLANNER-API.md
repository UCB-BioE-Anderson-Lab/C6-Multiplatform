...
*19 commands, generated from the header comment of each `bin/c6-*` by `bin/c6-commands`. What the flags MEAN is § 3.3, which is written by hand because no generator can know it.*
...
309 records in `sharables/generated/`, one per exported function that carries a JSDoc comment,
...
**Two types: `function` (262) and `datum` (47).** A `datum` is an exported constant that carries a
