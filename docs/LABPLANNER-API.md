...
282 records in `sharables/generated/`, one per exported function that carries a JSDoc comment,
...
**Two types: `function` (243) and `datum` (39).** A `datum` is an exported constant that carries a
...
|---|---|---|
| `plan` | 78 | the planning stages and their helpers — naming, vessels, sessions, sources, injectors, decisions |
