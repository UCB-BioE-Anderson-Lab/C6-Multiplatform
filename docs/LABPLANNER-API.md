...
285 records in `sharables/generated/`, one per exported function that carries a JSDoc comment,
...
**Two types: `function` (245) and `datum` (40).** A `datum` is an exported constant that carries a
...
|---|---|---|
| `plan` | 80 | the planning stages and their helpers — naming, vessels, sessions, sources, injectors, decisions |
