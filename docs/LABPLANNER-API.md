...
*16 commands, generated from the header comment of each `bin/c6-*` by `bin/c6-commands`. What the flags MEAN is § 3.3, which is written by hand because no generator can know it.*
...
**Two types: `function` (246) and `datum` (39).** A `datum` is an exported constant that carries a
