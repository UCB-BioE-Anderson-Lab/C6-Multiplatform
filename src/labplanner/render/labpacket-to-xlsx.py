...
#
# **TWO RULES, AND THIS FILE HELD THE WRONG ONE.** cloning-tutorials, planning/project_setup.md,
# keeps CONSTRUCT names to "4-6 characters to balance uniqueness with the ability to write it on a
# tube cap". A PCR tube LABEL is a different thing and shorter — JCA, 2026-09-12, of a column
# holding `pcr1`: *"pcr1 is a shitty name. It is above 3 letters max, which is a rule for pcr tube
# labels."* Three, because it goes on a 200 µL cap in marker, in gloves, eight times in a row.
#
# THE LABEL IS NOT THE CONSTRUCT NAME. `seq_pGhost12-A` is a fine name for the record and hopeless
# on a cap; `12` is the label and the two live side by side, in the columns the inventory already
# names — `label` and `construct`. The same trap produced a worse bug in the same sheet: labels
# generated by truncating the construct to three characters gave `pGh` twice and `412` twice, so
# two pairs of tubes were indistinguishable. Shortness is not the whole rule; uniqueness is the
# other half and the duplicate check below is the half that catches it.
#
# JCA, 2026-09-10, on a Zymo sheet reading `zseq_pGhost12-A`: *"You need to revisit the rules
# about labels. these are too long to write on a tube."*
LABEL_MAX = 3
